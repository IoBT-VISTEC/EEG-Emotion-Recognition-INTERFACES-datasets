...
#### LEAVE ONE CLIP OUT, USE ALL CHANNELS AND SOME FREQ
FreqSelect = [[0],[1],[2],[3] ,[0,1],[0,2],[0,3],[1,2],[1,3],[2,3],[0,1,2],[0,1,3],[0,2,3],[1,2,3]]

# prepare data for freq selection
DataX1 = EEG
DataX2 = E4
...
for j in FreqSelect:
    ...
    for i,x in enumerate (j):
        #print (i,x)        
        d1 = DataX1[:,:,x]
        d1 = d1.reshape([15,43,1])
        d2 = (DataX1[:,:,x+4])
        d2 = d2.reshape([15,43,1])
        d3 = (DataX1[:,:,x+8])
        d3 = d3.reshape([15,43,1])
        d4 = (DataX1[:,:,x+12])
        d4 = d4.reshape([15,43,1])
        d5 = (DataX1[:,:,x+16])
        d5 = d5.reshape([15,43,1])
        d6 = (DataX1[:,:,x+20])
        d6 = d6.reshape([15,43,1])
        d7 = (DataX1[:,:,x+24])
        d7 = d7.reshape([15,43,1])
        d8 = (DataX1[:,:,x+28])
        d8 = d8.reshape([15,43,1])
        d= np.concatenate((d1,d2,d3,d4,d5,d6,d7,d8), axis=2)
        if(i==0):
            D=d
        else:
            D=np.concatenate((D,d),axis=2)
    DataFq = np.asarray(D)
    DataAllFreq.append(DataFq)
    DataFq_E4.append(np.concatenate((DataFq,DataX2), axis=2))
    print (np.concatenate((DataFq,DataX2), axis=2).shape)

print('DataFq_E4 =', (len(DataFq_E4), len(DataFq_E4[0]), len(DataFq_E4[0][1]), ))
print('DataAllFreq =', (len(DataAllFreq), len(DataAllFreq[0]), len(DataAllFreq[0][1]), ))

# DataFq_E4: select some frequencies and concat with E4 features
# shape = (14, nclips, nsubjects, nfeatures)

# DataAllFreq: select some frequencies and concat without E4 features
# shape = (14, nclips, nsubjects, nfeatures)
        ...
        for dataset_id, X in enumerate(Data):
            ...
            for train_index, test_index in folds.split(X):
                ...
                if upsampling:
                    ...
                    if nc0==0 or nc1 == 0:
                        df = df.append({'dataset_id': FreqSelect[dataset_id], 'fold': test_index[0],
