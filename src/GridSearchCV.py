...
# specified source of labels (kmeans or threshold)
labelsType = "kmeans"
...
##### LEAVE ONE CLIP OUT, USE ALL CHANNELS AND FREQ (ALL FEATURES)

# emotion = 'Arousal', 'Valence'
for emotion in ['Arousal', 'Valence']:

    # reshape from (nsubjs * nclips, nfeatures) to (nsubjs, nclips, nfeatures)
    # ** SELECT f = 'EEG_only', 'E4_only', 'EEG_E4'
    for f in ['EEG_only', 'E4_only', 'EEG_E4']:
        if f == 'EEG_only':
            features = np.copy(EEG)
        elif f == 'E4_only':
            features = np.copy(E4)
        elif f == 'EEG_E4':
            features = np.copy(Fea)
        ...
        # uncomment here to label from threshold = 4.5 # 
        if labels_from == "threshold":
            f += '_th'
            if(emotion == 'Arousal'):
                y = np.load('../data/score/label/arousal.npy')
            elif(emotion == 'Valence'):
                y = np.load('../data/score/label/valence.npy')
            elif(emotion == 'Happy'):
                y = np.load('../data/score/label/happy.npy')
            elif(emotion == 'Fear'):
                y = np.load('../data/score/label/fear.npy')
            elif(emotion == 'Excite'):
                y = np.load('../data/score/label/excite.npy')
            elif(emotion == 'Reward'):
                y = np.load('../data/score/label/reward.npy')
        elif:
            f += '_kmeans'
            ...
            raise Exception("Please specify 'labels_from' = kmeans or threshold.")
        
        # ** setting upsampling test set or not
        upsampling = False

        X = np.copy(features)
        X = X.reshape(nsubjects, samples_per_subj, -1)
        y = y.reshape(nsubjects, samples_per_subj)
        print(X.shape, y.shape)
        assert all(features[1] == X[0][1])
        assert all(features[15] == X[1][0])

        # transpose to (nclips, nsubjs, nfeatures)
        X = np.transpose(X, (1, 0, 2))
        y = np.transpose(y)
        assert all(features[1] == X[1][0])
        assert all(features[15] == X[0][1])
        ...
        for train_index, test_index in folds.split(X):
            ...
            if upsampling:
                ...
                if nc0==0 or nc1 == 0:
                    df = df.append({'dataset_id': f, 'fold': test_index[0],
            ...
            print('clf_linear_best train correct: c0_train =', c0_train, '/', len(y_train[y_train==0]), 
                  'clf_linear_best c1_train =', c1_train, '/', len(y_train[y_train==1]), 'from', len(y_train), '=', train_acc)
            ...
            if clip_id == 9:
                np.save(emotion+'_y_pred', y_preds)
                break
                
        del features, X_train, y_train, X_test, y_test, folds, X, y
